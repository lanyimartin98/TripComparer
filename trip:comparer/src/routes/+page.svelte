...
{#if !closed}
	<article class="backdrop-blur-md p-4 border-2 m-2 rounded-md">
		<p class="lg:line-clamp-none {clamp} " on:click={expand}>
			...
			started, I want to disclose that this is just a part of my developer portfolio and not an
			actual production version. However, if you find it useful, I encourage you to consider
			donating to the API makers FlightRadar and Carbon Interface, who have provided the underlying
			technology that powers this application. Your support will help them continue to innovate and
			make their APIs available for developers like myself to use in creating useful tools like this
			one. If you have any non-API related questions, comments, or feedback, please feel free to
			reach out to me on LinkedIn or Github. I am always interested in connecting with other
			developers and learning from their experiences. Thank you for taking the time to try out this
		...
		<p class="p-1">
			ps.: The introduction was actually generated by ChatGPT, just to throw in some fancy AI
			stuff...
		</p>
		<p class="text-right">
			<button on:click={close}><i class="bi bi-arrows-angle-contract" /></button>
		</p>
	</article>
{:else}
	<article class="backdrop-blur-md p-2 border-2 m-2 rounded-md flex flex-row justify-between">
		Introduction <button on:click={close}><i class="bi bi-chevron-expand" /></button>
	</article>
{/if}
